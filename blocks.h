	...
	*/
	...
	{" ", "echo 🔭$(cat /proc/loadavg | awk '{ print $1 }') \
		$(free -h | awk '/^Mem/ { print $3 }' | sed s/i//g)", 3, 0},
													   //
	{"", "echo -n $(newsboat -x print-unread | awk '{ if($1>0) print \"📰\" $1 }')", 60, 0},

	{"", "echo -n $(find \"${XDG_DATA_HOME:-$HOME/.local/share}\"/mail/*/INBOX/new/* -type f | \
		wc -l 2>/dev/null | awk '{ if($1>0) print \"📬\" $1 }')", 60, 0},

	/* {"🔋", "echo -n $(cat /sys/class/power_supply/BAT0/capacity)%", 60, 0}, */
	{"", "echo -n $(if [ $(cat /sys/class/power_supply/BAT0/status) == 'Charging' ]; then echo ⚡; else echo 🔋; fi)$(cat /sys/class/power_supply/BAT0/capacity)", 60, 0},
	...
	{"", "curl -s 'wttr.in/Lokeren?format=%m'", 21600, 0},
